...
#    _sruns = samples.loc[(samples['sampid'] == wildcards.sampid) & (samples['assay'] == 'RNA-Seq'), 'runid']
    ...
    if _layouts[0] == 'SINGLE':
        return expand("results/fastq/{runid}/{runid}_1.fastq.gz", runid=_sruns)
    elif _layouts[0] == 'PAIRED':
        return expand("results/fastq/{runid}/{runid}_{rn}.fastq.gz", runid=_sruns, rn=[1,2])
    else:
...
def readfiles_param(wildcards):
    _sruns = samples.loc[wildcards.sample_id]['runid']
#    _sruns = samples.loc[(samples['sampid'] == wildcards.sample_id) & (samples['assay'] == 'RNA-Seq'), 'runid']
    _layouts = runs.loc[_sruns]['layout']
    if not all(_layouts[0] == _ for _ in _layouts):
        msg = f'All runs for sample "{wildcards.sample_id}" do not have the same layout (PAIRED or SINGLE)'
        raise WorkflowError(msg)
